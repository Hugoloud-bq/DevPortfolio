...
@if(request('search') || request('status'))
    <div class="mb-3">
        <a href="{{ route('projects') }}" class="btn btn-sm btn-outline-secondary">
            <i class="fas fa-times me-1"></i> Очистить фильтры
        </a>
    </div>
@endif

<!-- Фильтр по статусу (с подсветкой) -->
<div class="mb-3">
    <span class="me-2">Фильтр по статусу:</span>
    <a href="{{ route('projects', ['sort' => request('sort'), 'search' => request('search')]) }}" class="btn btn-sm {{ !request('status') ? 'btn-primary' : 'btn-outline-secondary' }}">Все</a>
    <a href="{{ route('projects', ['status' => 'tz', 'sort' => request('sort'), 'search' => request('search')]) }}" class="btn btn-sm {{ request('status') == 'tz' ? 'btn-success' : 'btn-outline-secondary' }}">ТЗ сдано</a>
    <a href="{{ route('projects', ['status' => 'report', 'sort' => request('sort'), 'search' => request('search')]) }}" class="btn btn-sm {{ request('status') == 'report' ? 'btn-info' : 'btn-outline-secondary' }}">Отчёт сдан</a>
    <a href="{{ route('projects', ['status' => 'diary', 'sort' => request('sort'), 'search' => request('search')]) }}" class="btn btn-sm {{ request('status') == 'diary' ? 'btn-warning' : 'btn-outline-secondary' }}">Дневник сдан</a>
</div>
...
<div class="mb-3 text-end">
    <a href="{{ route('projects.export-excel') }}" class="btn btn-success me-2">
        <i class="fas fa-file-excel me-1"></i> Экспорт в Excel
    </a>
<a href="{{ route('projects.export-pdf', ['theme' => session('theme', 'light')]) }}" class="btn btn-danger">
    <i class="fas fa-file-pdf me-1"></i> Экспорт в PDF
</a>
</div>

<!-- Счётчик найденных проектов -->
<div class="mb-3">
    <small class="text-muted">Найдено проектов: {{ count($projects) }}</small>
</div>
